...
async def send_message(request: Request, body: ChatRequest):
    ...
    cleaned = post_process(re.sub(r'\s*$.*?$\s*', '', response_text).strip())

    return ChatResponse(text=cleaned, emotion=output_emotion, sources=sources)
...
async def stream_message(request: Request, body: ChatRequest):
    ...
    async def generate():
        ...
        # Kuyrukta bekleme varsa bildir
        q_size = queue_manager.queue_size()
        if q_size > 0:
            queue_info = {
                "position": q_size + 1,
                "estimated_seconds": queue_manager.estimated_wait()
            }
            yield f"[QUEUE]{json.dumps(queue_info)}[/QUEUE]"
        ...
        try:
            ...
            queue_manager.release(time.time() - start_time)
